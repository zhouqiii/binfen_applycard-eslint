<template>
    <div>
        <nav-bar-base title="我要申请-第三步" ref="nav">
        </nav-bar-base>
        <div>
...
<script>
import pic from '@/assets/images/card3.png';
import { Form, Switch, Cell } from 'vant';
...
export default {
  ...
  methods: {
    ...
    checkPhone() {
      ...
      } if (!re.test(this.formData.emergencyConPhone)) {
        this.errMsg.mobilePhone = '格式错误！';
        this.flag.emergencyConPhone = false;
      } if (this.formData.emergencyConPhone.length > 11) { // 限制电话号码在7-11位
    ...
    checkName() {
      ...
      if (!this.formData.emergencyContact) { // 不能为空
        ...
        return false;
      } if (!chinese.test(this.formData.emergencyContact)) { // 姓名只能为汉字
    ...
    sendData(data) {
      ...
      this.requestAxios({
        ...
        params: {
          ...
          picId: parseInt(this.$route.query.picId, 10),
          // checkResult: 'll', // 审核结果
          ...
          userId: 123456, // 用户id
          cardId: parseInt(this.$route.query.cardId, 10), // 信用卡id
        ...
        .then((res) => {
          if (JSON.parse(res).stat === '00') {
            this.$router.push({
              path: '/ApplyEnd',
              query: {
                data: JSON.stringify(JSON.parse(res).result),
              },
            });
          } else {
            this.$router.push({
    ...
    submitMsg() {
      ...
      const allData = {};// 把所有信息都存在allData对象里，在allData里和后台对应字段，并把allData传给后台
      ...
      const basicData = JSON.parse(sessionStorage.getItem('basicData'));// 把申请第一步的基本信息取出
      const otherData = JSON.parse(sessionStorage.getItem('otherData'));// 把申请第二步的补充信息取出并转为对象
