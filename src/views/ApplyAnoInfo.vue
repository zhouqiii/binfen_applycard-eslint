<template>
    <div class="home">
        <nav-bar-base title="我要申请-第二步" ref="nav">
        </nav-bar-base>
        <div>
...
import '../assets/css/style/applyAnoInfo.less';
  ...
  methods: {
    ...
    checkComPhone() {
      ...
      if (!this.formData.companyNumber) {
        ...
        return false;
      } if (!re.test(this.formData.companyNumber)) {
    ...
    checkContact() {
      ...
      if (!this.formData.contactUrgent) {
        ...
        return false;
      } if (!chinese.test(this.formData.contactUrgent)) { // 判断字符串是否为汉字
    ...
    // 数据赋值
    getHomeDataSucc(res) {
      const cityList = res.city;
      if (cityList) {
        this.options = cityList;
        this.optionsComCity = cityList;
      }
    },
  },
  mounted() {
